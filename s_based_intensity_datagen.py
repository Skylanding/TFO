...
#from tools.intensity_datagen import intensity_from_raw
from tools.name_decoder import decode_extended_filename
...
def get_mu_a(saturation: float, concentration: float, wave_int: int) -> float:
    ...
    mu_a = 2.303 * concentration * \
        (saturation * E_HB[wave_index] + (1 - saturation)
         * E_HBO2[wave_index])  # in cm-1
                ...
                for s_m in s_m_list:
                    ...
                    for c_f in c_f_list:
                        for s_f in s_f_list:
                            ...
                            intensity_df['Wave Int'] = wave_int * \
                                np.ones((num_rows, 1))
                            intensity_df['Uterus Thickness'] = uterus_thickness * \
                                np.ones((num_rows, 1))
                            intensity_df['Maternal Wall Thickness'] = maternal_wall_thickness * \
                                np.ones((num_rows, 1))
                            intensity_df['Maternal Hb Concentration'] = c_m * \
                                np.ones((num_rows, 1))
                            intensity_df['Maternal Saturation'] = s_m * \
                                np.ones((num_rows, 1))
                            intensity_df['Fetal Hb Concentration'] = c_f * \
                                np.ones((num_rows, 1))
                            intensity_df['Fetal Saturation'] = s_f * \
                                np.ones((num_rows, 1))
