...
for i=1:20
    ...
    cfg.prop=[0 0 1 1            % medium 0: the environment
       0.009 1.2  0.9  1.4     % medium 1: Maternal abdominal wall
       0.01 0.815    0.9  1.4     % medium 2: Maternal uterus
       0.004 0.01  0.9  1.334     % medium 3: Amniotic fluid
       0.013 0.9916    0.9  1.4];   % medium 4: Fetal tissues
    ...
    cfg.detpos=[(10+4*i)*precision 50*precision 1*precision 1.5*precision];
    tic;
    [f2,det2]=mcxlab(cfg);
    toc;
...
%     title(sprintf('detected %d photons',size(det2.ppath,1)));
    ...
    attenuation_t(i)=size(det2.ppath,1)/cfg.nphoton;
